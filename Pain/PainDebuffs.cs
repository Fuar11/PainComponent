...
namespace PainComponent.Pain
{
    internal class PainDebuffs
    {
        ...
        public class UpdateCraftingDuration
        {
            ...
            {
                float multi = UtilityFunctions.GetCraftingDurationMultiplier();
        ...
        internal static class CraftingOperationApplyProgressPatch
        {
            ...
            {
                UtilityFunctions.ScaleCraftingProgressHours(ref hoursSpentCrafting);
            }
        }
        ...
        public class MovementSpeedModifier
        {
            public static void Postfix(ref float __result)
            {
                ...
                else
                {
                    ...
                    {
                        __result /= pm.IsOverdosing() ? 0.7f : 0.9f;
                    }
                }
            }
        }
        ...
        public class ReadingChanges
        {
            ...
            private static bool ShouldPreventReading()
            {
                ...
                if (GameManager.GetConditionComponent().HasNonRiskAffliction())
                {
                    ...
                    List<CustomPainAffliction> painAfflictions = AfflictionHelper.GetPainAfflictions("Concussion");

                    if (painAfflictions.Count > 0)
                    {

                        if (!Mod.painManager.PainkillersInEffect(painAfflictions.ElementAt(0).m_PainLevel))
                        {
        ...
        public class PainSprintRestriction
        {
            ...
            public static void Postfix(ref bool __result)
            {
                ...
                if (pm == null) return;
                else if (pm.am == null) return;

                if (pm.am.GetAfflictionsOfType(typeof(CustomPainAffliction)).Any(aff => aff.HasLimp()))
                {
                    __result = true;
                }
        ...
        [HarmonyPatch(typeof(SprainedWrist), nameof(SprainedWrist.CanEquipItem))]

        public class EquipRestriction
        {

            public static void Postfix(ref GearItem gi, ref SprainedWrist.CanEquipItemResult __result)
            {
